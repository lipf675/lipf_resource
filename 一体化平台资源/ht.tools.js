...
var AgentClient = function(option) {
	...
	var config = {
		...
		autoReady : 1,	// 自动空闲, 1:事后整理(默认), 2:自动置闲, 3:自动置忙
		acwTime : 0,	// 事后整理时长(毫秒)
	...
	function getOption(field) {
		if (field == undefined) {
			...
			return agent[field] || config[field];
		}
	}
	...
	function listenCall(destType, destDesc) {
		if (agent.isLogin) {
			return sendCommand(createListenCommand(agent.agentId, agent.agentDn, destType, destDesc));
		}
		return false;
	}
	...
	function createListenCommand(agentId, agentDn, destType, destDesc) {
		return {
	...
	var APPOINT_TYPE_NOTREADY = 1; // 预约置忙
	...
	var AUTO_TYPE_AGENTREADY = 2; // 自动置闲
	var AUTO_TYPE_AGENTNOTREADY = 3; // 自动置忙
...
(function(){
	...
	function $package(name){
  	...
  	window["cn"]["htinfo"]["namespace"] = $package;
  
})();

cn.htinfo.namespace("cn.htinfo.platform.eform.tools.api");
cn.htinfo.platform.eform.tools.api = function(config){
	this.agentId=config.agentId;
	this.agentDn=config.agentDn;
	...
	var self = this;
	//响铃事件
	this.client.onOffering= function(event) {
		if (self.isFunction(self.onOffering)) {
			self.onOffering(event);
		}
		if(self.deflectOpt.isDeflect&&self.deflectOpt.destType&&self.deflectOpt.destDesc){
			self.deflectCall(self.deflectOpt.destType, self.deflectOpt.destDesc);
		}
	};
	
	this.client.onOnline= function(event) {
		if (self.isFunction(self.onOnline)) {
			self.onOnline(event);
		}
	};

	this.client.onOffline= function(event) {
		if (self.isFunction(self.onOffline)) {
			self.onOffline(event);
		}
	};
	this.client.onDialing= function(event) {
		if (self.isFunction(self.onDialing)) {
			self.onDialing(event);
		}
	};
	this.client.onConnected= function(event) {
		if (self.isFunction(self.onConnected)) {
			self.onConnected(event);
		}
	};
	this.client.onReleased= function(event) {
		if (self.isFunction(self.onReleased)) {
			self.onReleased(event);
		}
	};
	this.client.onRecordEnd= function(event) {
		if (self.isFunction(self.onRecordEnd)) {
			self.onRecordEnd(event);
		}
	};
	this.client.onError= function(event) {
		if (self.isFunction(self.onError)) {
			self.onError(event);
		}
	};
	this.client.onStatusChanged= function(event) {
		if (self.isFunction(self.onStatusChanged)) {
			self.onStatusChanged(event);
		}
	};
};

cn.htinfo.platform.eform.tools.api.prototype={
	//初始化
	...
	setOption:function(option) {
		return this.client.setOption(option);
	},
	//开始
	start:function(){
	...
	//小休
	otherWork:function(){
		return this.client.otherWork(0);
	},
	//外呼
	makeCall:function(destType, destDesc, callerId) {
		return this.client.makeCall(destType, destDesc, callerId);
	},
	//应答
	answerCall:function(){
		return this.client.answerCall();
	},
	//挂机
	hangupCall:function(){
		return this.client.hangupCall();
	},
	//呼叫保持
	holdCall:function(){
		return this.client.holdCall();
	},
	//取回呼叫
	retrieveCall:function(){
		return this.client.retrieveCall();
	},
	//转IVR
	transferIVR:function(flowFlag, callData) {
		return this.client.transferIVR(flowFlag, callData);
	},
	//通话状态下转接
	transferCall:function(destType, destDesc, callerId) {
		return this.client.transferCall(destType, destDesc, callerId);
	},
	//振铃状态下转接
	deflectCall:function(destType, destDesc) {
		return this.client.deflectCall(destType, destDesc);
	},
	//监听
	listenCall:function(destType, destDesc) {
		return this.client.listenCall(destType, destDesc);
	},
	//强拆
	forceHangupCall:function(destType, destDesc) {
		return this.client.forceHangupCall(destType, destDesc);
	},
	
	//代接
	pickupCall:function(destType, destDesc) {
		return this.client.pickupCall(destType, destDesc);
	},
	
	//设置振铃转接
	setDeflect:function(destType, destDesc){
		this.deflectOpt.isDeflect = true;
		this.deflectOpt.destType = destType;
		this.deflectOpt.destDesc = destDesc;
	},
...
$voiceApiFactory.getTool = function(config){
	return new cn.htinfo.platform.eform.tools.api({
